/**
 * @file
 * @brief Contains the Challenge12 class implementation.
 * @author Alunya
 * @date 12.12.2025
 */
 ...
 */
Challenge12::Challenge12(const std::filesystem::path& filePath)
        : BaseChallenge(filePath)
{
    if (!ReadFile()) {
        std::println("Challenge 12:: Initialisation failed: {} not loaded", m_FilePath.string());
        return;
    }

    std::println("Challenge 12:: Initialised with file: {}", m_FilePath.string());
}
 ...
 */
ChallengeResult Challenge12::RunChallenge()
{
...
std::int64_t Challenge12::PartII()
{
    std::int64_t result{};
