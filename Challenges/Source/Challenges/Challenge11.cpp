/**
 * @file
 * @brief Contains the Challenge11 class implementation.
 * @author Alunya
 * @date 11.12.2025
 */
 ...
 */
Challenge11::Challenge11(const std::filesystem::path& filePath)
        : BaseChallenge(filePath)
{
    if (!ReadFile()) {
        std::println("Challenge 11:: Initialisation failed: {} not loaded", m_FilePath.string());
        return;
    }

    std::println("Challenge 11:: Initialised with file: {}", m_FilePath.string());
}
 ...
 */
ChallengeResult Challenge11::RunChallenge()
{
...
std::int64_t Challenge11::PartII()
{
    std::int64_t result{};
