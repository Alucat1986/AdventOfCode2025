/**
 * @file
 * @brief Contains the Challenge10 class implementation.
 * @author Alunya
 * @date 10.12.2025
 */
 ...
 */
Challenge10::Challenge10(const std::filesystem::path& filePath)
        : BaseChallenge(filePath)
{
    if (!ReadFile()) {
        std::println("Challenge 10:: Initialisation failed: {} not loaded", m_FilePath.string());
        return;
    }

    std::println("Challenge 10:: Initialised with file: {}", m_FilePath.string());
}
 ...
 */
ChallengeResult Challenge10::RunChallenge()
{
...
std::int64_t Challenge10::PartII()
{
    std::int64_t result{};
