/**
 * @file
 * @brief Contains the Challenge09 class implementation.
 * @author Alunya
 * @date 09.12.2025
 */
 ...
 */
Challenge09::Challenge09(const std::filesystem::path& filePath)
        : BaseChallenge(filePath)
{
    if (!ReadFile()) {
        std::println("Challenge 09:: Initialisation failed: {} not loaded", m_FilePath.string());
        return;
    }

    std::println("Challenge 09:: Initialised with file: {}", m_FilePath.string());
}
 ...
 */
ChallengeResult Challenge09::RunChallenge()
{
...
std::int64_t Challenge09::PartII()
{
    std::int64_t result{};
